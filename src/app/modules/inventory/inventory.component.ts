...
export class InventoryComponent implements OnInit {
  ...
  onDelete(_id) {
    this.dialogService.openDialog('Are you sure to delete this material?')
                      .afterClosed().subscribe(res => {
                        if(res){
                          this.service.deleteMaterial(_id).subscribe(data => {
                            console.log(data)
                          })
                          this.notificationService.warn('Successfully deleted material')
                        }
